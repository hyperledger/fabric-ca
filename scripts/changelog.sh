...
echo "## $3\n$(date)\n" >> CHANGELOG.new
git log $1..$2  --oneline | grep -v Merge | sed -e "s/\[\(FAB-[0-9]*\)\]/\[\1\](https:\/\/jira.hyperledger.org\/browse\/\1\)/" -e "s/\([0-9|a-z]*\)/* \[\1\](https:\/\/github.com\/hyperledger\/fabric-ca\/commit\/\1)/" >> CHANGELOG.new
echo "" >> CHANGELOG.new
