...
function setupTLScerts() {
  ...
  # EE TLS certs
  i=0
  while test $((i++)) -lt $((NUMINTCAS + 1)); do
...
function createRootCA() {
  if [[ ${driver} == "mysql" ]]; then
    TLS_ROOT=${FABRIC_CA}/${MYSQL_CERTS}/ca.pem
    TLS_KEY=${FABRIC_CA}/${MYSQL_CERTS}/client-key.pem
    TLS_CERT=${FABRIC_CA}/${MYSQL_CERTS}/client-cert.pem
  else
    TLS_ROOT=$FABRIC_CA_DATA/$TLS_BUNDLE
    TLS_KEY=$PGSSLKEY
    TLS_CERT=$PGSSLCERT
  fi
  # Start RootCA
  $($FABRIC_TLS) && tlsopts="--tls.enabled \
                              ...
                              --db.tls.certfiles $TLS_ROOT \
                              --db.tls.client.certfile $TLS_CERT \
                              --db.tls.client.keyfile $TLS_KEY"
  mkdir -p "$TDIR/root"
...
function createIntCA() {
  ...
  i=0
  while test $((i++)) -lt $NUMINTCAS; do
    mkdir -p "$TDIR/int${i}"
    cp "$TDIR/intFabricCaFvt.yaml" "$TDIR/int${i}/runFabricCaFvt.yaml"
    $($FABRIC_TLS) && tlsopts="--tls.enabled \
                                 --tls.certfile $TLSDIR/intFabCaTls${i}-cert.pem \
...
function createFailingCA() {
  if [[ ${driver} == "mysql" ]]; then
    TLS_ROOT=${FABRIC_CA}/${MYSQL_CERTS}/ca.pem
    TLS_KEY=${FABRIC_CA}/${MYSQL_CERTS}/client-cert.pem
    TLS_CERT=${FABRIC_CA}/${MYSQL_CERTS}/client-key.pem
  else
    TLS_ROOT=$FABRIC_CA_DATA/$TLS_BUNDLE
    TLS_KEY=$PGSSLKEY
    TLS_CERT=$PGSSLCERT
