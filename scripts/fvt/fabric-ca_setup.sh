...
resetFabricCa() {
  ...
  for i in "" $(seq ${CACOUNT:-0}); do
    ...
    mysql --host=${MYSQL_HOST} --user=root --password=mysql -e 'show tables' ${DBNAME}${dbSuffix} >/dev/null 2>&1
    mysql --host=${MYSQL_HOST} --user=root --password=mysql -e "DROP DATABASE IF EXISTS ${DBNAME}${dbSuffix}" >/dev/null 2>&1
...
listFabricCa() {
  ...
  for i in "" $(seq ${CACOUNT:-0}); do
    ...
    mysql)
      echo ""
      echo "Users:"
      mysql --host=${MYSQL_HOST} --user=root --password=mysql -e 'SELECT * FROM users;' ${DBNAME}${dbSuffix}
      if $($FABRIC_CA_DEBUG); then
        echo "Certificates:"
        mysql --host=${MYSQL_HOST} --user=root --password=mysql -e 'SELECT * FROM certificates;' ${DBNAME}${dbSuffix}
        echo "Affiliations:"
        mysql --host=${MYSQL_HOST} --user=root --password=mysql -e 'SELECT * FROM affiliations;' ${DBNAME}${dbSuffix}
      fi
      ;;
    postgres)
      echo ""
...
mysql) DATASRC="root:mysql@tcp($MYSQL_HOST:$MYSQL_PORT)/$DBNAME?parseTime=true$mysqlTls" ;;
esac
