...
db:
  type: mysql
  datasource: root:mysql@tcp($MYSQL_HOST:$MYSQL_PORT)/fabric_ca$mysqlTls
  tls:
     enabled: $FABRIC_TLS
     certfiles:
       - ${FABRIC_CA}/${MYSQL_CERTS}/ca.pem
     client:
       certfile: ${FABRIC_CA}/${MYSQL_CERTS}/client-cert.pem
       keyfile: ${FABRIC_CA}/${MYSQL_CERTS}/client-key.pem
...
db:
  type: postgres
  datasource: host=localhost port=$POSTGRES_PORT user=postgres password=postgres dbname=fabric_ca $postgresTls
  tls:
     enabled: $FABRIC_TLS
     certfiles:
       - $TLS_ROOTCERT
...
EOF
  ...
  # mysql configuration with two bootstrap users
  cp $PGSQLSERVERCONFIG $PGSQLSERVERCONFIG2
  sed -i '/hf.IntermediateCA:/a\
     - name: c\
       pass: d\
       type: client\
       ...
       attrs:\
          ...
          hf.Registrar.DelegateRoles: "client,user,validator,auditor"\
          hf.Revoker: true\
          hf.IntermediateCA: true ' $PGSQLSERVERCONFIG2
  echo -e "   org3:\n      - department1" >>$PGSQLSERVERCONFIG2
}
...
mysql --host=${MYSQL_HOST} --user=root --password=mysql -e "drop database $DBNAME;" -e "create database $DBNAME;" &>/dev/null
mysql --host=${MYSQL_HOST} --user=root --password=mysql --database=$DBNAME -e "CREATE TABLE users (id VARCHAR(64) NOT NULL, token blob, type VARCHAR(64), affiliation VARCHAR(64), attributes VARCHAR(256), state INTEGER, max_enrollments INTEGER, PRIMARY KEY (id)) DEFAULT CHARSET=utf8 COLLATE utf8_bin;" &>/dev/null
...
mysql --host=${MYSQL_HOST} --user=root --password=mysql -e "drop database fabric_ca;" -e "create database fabric_ca;" &>/dev/null
...
mysql --host=${MYSQL_HOST} --user=root --password=mysql -e "drop database fabric_ca;" &>/dev/null
