...
function mysqlDBCleanup() {
  mysql --host=${MYSQL_HOST} --user=root --password=mysql --database=$DBNAME -e "TRUNCATE TABLE certificates" &>/dev/null
}
...
function insertMySQLCertsTable() {
  ...
  # Store the generated certificate in the certificates table
  mysql --host=${MYSQL_HOST} --user=root --password=mysql --database=$DBNAME -e "INSERT INTO certificates (id, serial_number, authority_key_identifier, ca_label, status, reason, expiry, revoked_at, pem, level) VALUES ('$id', '$serial', '$aki', 'ca', 'active', '0', '$expiry', '$revokedAt', '$pem', '1')"
}
