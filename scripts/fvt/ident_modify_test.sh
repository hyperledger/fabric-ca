...
function getAttrs() {
  ...
  $FABRIC_CA_CLIENTEXEC identity list $URI -H $TESTDIR/$admin | grep -oP 'everythingBagel.*?]' | tr '{' "\n" | grep hf | sort | awk 'BEGIN {print ""}; {print $1" "$2}' | tr -d "'"
}
...
function testAuthenticationAuthorization() {
  ...
  mv $HOME/admincert.pem $TESTDIR/admin/msp/signcerts/cert.pem
  mv $HOME/adminkey.pem $TESTDIR/admin/msp/keystore/key.pem
  ...
  for op in list remove add modify; do
    ...
    # Unknown CA
    $FABRIC_CA_CLIENTEXEC identity $op $user $URI -d -H $TESTDIR/admin >caoutput 2>&1
    # @TODO these messages need to change
    cat caoutput
    egrep "Authentication failure" caoutput || ErrorMsg "Test '$op' Authorization"
    # testUser not authorized - user must have the "hf.Registrar.Roles" attribute
    $FABRIC_CA_CLIENTEXEC identity $op $user $URI -d -H $TESTDIR/testUser >caoutput 2>&1
    # @TODO these messages need to change
    egrep "Authorization failure" caoutput || ErrorMsg "Test '$op' Authorization"
  done
}
...
function testRoleAuthorization() {
  ...
  $FABRIC_CA_CLIENTEXEC identity add userType1 $URI -H $TESTDIR/admin \
    --type account --affiliation ${defaultValues[Affiliation]} >caoutput 2>&1
  egrep "Authorization failure" caoutput ||
  ...
  for i in $(seq $NUMROLES); do
    ...
    $FABRIC_CA_CLIENTEXEC identity list $URI -H $TESTDIR/userType$i >caoutput 2>&1
    egrep "hf.EnrollmentID Value:userType$i.*Type Value:role$i.*Affiliation Value: " caoutput ||
...
function testModifyRegistrarRoles() {
  ...
  $FABRIC_CA_CLIENTEXEC identity modify userType1 $URI -d \
    -H $TESTDIR/admin2 --type client >caoutput 2>&1
  egrep "Authorization failure" caoutput ||
...
function testDelegation() {
  ...
  $FABRIC_CA_CLIENTEXEC identity add userType10 $URI -d -H $TESTDIR/admin \
    ...
    --attrs '"hf.Registrar.DelegateRoles=type10"' >caoutput 2>&1
  egrep "Authorization failure" caoutput ||
  ...
  $FABRIC_CA_CLIENTEXEC identity add SuperUser $URI -d \
    --json "$SuperAttrs" -H $TESTDIR/restrictedAdmin >caoutput 2>&1
  egrep "Authorization failure" caoutput ||
...
function testLateralAffiliation() {
  ...
  $FABRIC_CA_CLIENTEXEC identity modify $user $URI -d \
    ...
    --attrs '"hf.Registrar.Roles=client,user,peer,validator,auditor,ca,app,role1,role2,role3,role4,role5,role6,role7,role8,apple,orange,ca"' >caoutput 2>&1
  egrep "Authorization failure" caoutput || ErrorMsg "$admin should not be able to operate on higher level affiliation ${defaultValues[Affiliation]}"
  ...
  $FABRIC_CA_CLIENTEXEC identity modify $user $URI -d -H $TESTDIR/$admin \
    --attrs "hf.IntermediateCA=false" >caoutput 2>&1
  egrep "Authorization failure" caoutput ||
  ...
  eval "userDef=\"$adminTemplate\""
  $FABRIC_CA_CLIENTEXEC identity add $user $URI -d --json "$userDef" -H $TESTDIR/$admin >caoutput 2>&1
  egrep "Authorization failure" caoutput ||
...
function testConflictingHfAttrs() {
  ...
  $FABRIC_CA_CLIENTEXEC identity add ${user}1 $URI -d --json "$userdef" -H $TESTDIR/$admin >caoutput 2>&1
  egrep "Authorization failure" caoutput ||
  ...
  userdef='
   ...
   '
  $FABRIC_CA_CLIENTEXEC identity add ${user}2 $URI -d --json "$userdef" -H $TESTDIR/admin2 >caoutput 2>&1
  egrep "Authorization failure" caoutput ||
...
function removeAllUsers() {
  ...
  $FABRIC_CA_CLIENTEXEC identity remove id $URI -H $TESTDIR/admin/ >caoutput 2>&1
  egrep 'Failed to get User' caoutput ||
    ErrorMsg "Should have failed, or wrong error code"
  # attempt delete self w/o force
  $FABRIC_CA_CLIENTEXEC identity remove admin $URI -H $TESTDIR/admin/ >caoutput 2>&1
  egrep "Need to use 'force'" caoutput ||
    ErrorMsg "Should have failed, or wrong error code"
  # delete self
