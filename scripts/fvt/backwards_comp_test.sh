...
db:
  type: mysql
  datasource: root:mysql@tcp(mysql:$MYSQL_PORT)/$DBNAME
...
function resetDB {
  case "$driver" in
    ...
    mysql)
      mysql --host=mysql --user=root --password=mysql -e "DROP DATABASE $DBNAME" ;;
    *)
...
function createDB {
  case "$driver" in
    ...
    mysql)
      mysql --host=mysql --user=root --password=mysql -e "CREATE DATABASE $DBNAME" ;;
    *)
      echo "Invalid database type"
      exit 1
      ;;
...
function loadUsers {
  case "$driver" in
    ...
    mysql)
      mysql --host=mysql --user=root --password=mysql -e "CREATE DATABASE $DBNAME"
      mysql --host=mysql --user=root --password=mysql --database=$DBNAME -e "CREATE TABLE IF NOT EXISTS users (id VARCHAR(255) NOT NULL, token blob, type VARCHAR(256), affiliation VARCHAR(1024), attributes TEXT, state INTEGER, max_enrollments INTEGER, PRIMARY KEY (id)) DEFAULT CHARSET=utf8 COLLATE utf8_bin"
      mysql --host=mysql --user=root --password=mysql --database=$DBNAME -e "INSERT INTO users (id, token, type, affiliation, attributes, state, max_enrollments) VALUES ('registrar', '', 'user', 'org2', '[{\"name\": \"hf.Registrar.Roles\", \"value\": \"user,peer,client\"},{\"name\": \"hf.Revoker\", \"value\": \"true\"}]', '0', '-1')"
      mysql --host=mysql --user=root --password=mysql --database=$DBNAME -e "INSERT INTO users (id, token, type, affiliation, attributes, state, max_enrollments) VALUES ('notregistrar', '', 'user', 'org2', '[{\"name\": \"hf.Revoker\", \"value\": \"true\"}]', '0', '-1')"
      ;;
    *)
      echo "Invalid database type"
      exit 1
...
function validateUsers {
  ...
  case "$driver" in
    ...
    postgres)
      ...
      ;;
    mysql)
      mysql --host=mysql --user=root --password=mysql --database=$DBNAME -e "SELECT attributes FROM users WHERE (id = 'registrar')" | grep '"name":"hf.Registrar.Attributes","value":"*"'
      if test $? -eq 1; then
        ErrorMsg "Failed to correctly migrate user 'registrar' on mysql"
      fi
      mysql --host=mysql --user=root --password=mysql --database=$DBNAME -e "SELECT attributes FROM users WHERE (id = 'notregistrar')" | grep '"name":"hf.Registrar.Attributes","value":"*"'
      if test $? -eq 0; then
        ErrorMsg "Failed to correctly migrate user 'notregistrar' on mysql"
      fi
      mysql --host=mysql --user=root --password=mysql --database=$DBNAME -e "SELECT attributes FROM users WHERE (id = 'a')" | grep '"name":"hf.Registrar.Attributes","value":"*"'
      if test $? -eq $result; then
...
for driver in sqlite3 postgres mysql; do
  ...
  postgres)
    ...
    ;;
  mysql)
    mysql --host=mysql --user=root --password=mysql -e "DROP DATABASE fabric_ca"
    mysql --host=mysql --user=root --password=mysql -e "CREATE DATABASE fabric_ca"
    mysql --host=mysql --user=root --password=mysql --database=fabric_ca -e "CREATE TABLE IF NOT EXISTS properties (property VARCHAR(255), value VARCHAR(256), PRIMARY KEY(property))"
    mysql --host=mysql --user=root --password=mysql --database=fabric_ca -e "INSERT INTO properties (property, value) Values ('identity.level', '9')"
    ;;
  *)
    echo "Invalid database type"
    exit 1
